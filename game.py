...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_games_per_MCTS_process", type=int, default=50, help="Number of games to simulate per MCTS self-play process")
    ...
    #run_MCTS(args, start_idx=0, iteration=2)
    train_connectnet(args, iteration=2, new_optim_state=True)
    winner = evaluate_nets(args, iteration_1=0, iteration_2=2)
    ...
    #         else:
    #             winner = evaluate_pos(args, 1, env.game)
    #             action = np.argmax(winner)
    #             if env.game.player == 0:
    #                 print("player (2), MCTS decision:", action+1, winner)
